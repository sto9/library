...
// a-b-c の角度（[0,2π)、aを反時計回りに回転させてcに重ねる角度）
Real get_angle2(const Point &a, const Point &b, const Point &c)
{
  const Point v(a - b), w(c - b);
  Real theta = atan2(imag(w), real(w)) - atan2(imag(v), real(v));
  while (theta < 0)
    theta += PI * 2;
  while (theta > PI * 2)
    theta -= PI * 2;
...
// --------------------
void clean(Polygon &p)
{
  p = convex_hull(p);
  Polygon temp;
  rep(i, p.size())
  {
    if (i >= 1 and fabs(p[i] - p[i - 1]) < EPS)
      continue;
    temp.push_back(p[i]);
  }
  p = temp;
...
Polygon intersection(const Polygon &p1, const Polygon &p2)
{
  ...
  rep(i, n1) rep(j, n2)
  {
    ...
    if (intersect(s1, s2))
      res.push_back(crosspoint(s1, s2));
  }
  clean(res);
  return res;
}
